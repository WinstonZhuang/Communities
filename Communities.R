...
library(tmaptools)
library(ggmap)
api_key <- "bea0cba92dad47ed99dd9f34bd5532e7"
...
server <- function(input, output, session) {
  ...
  data <- reactive({
    ...
    latlon <- geocode_OSM(com$Names)
    com <- cbind(com, latlon)
    com[, selected()] <- as.numeric(com[, selected()])
  ...
  output$map <- renderLeaflet({
    ...
    leaflet(data = nyc_shapefile) %>%
      addTiles() %>%
      addPolygons(fillColor = ~palette(as.numeric(data()[[input$type]])), 
                  fillOpacity = 0.7, 
                  color = "black",
                  weight = 1) %>%
